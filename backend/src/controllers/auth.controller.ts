...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    const isLocal = req.headers.host?.includes('172.28.91.10') || req.headers.host?.includes('10.91.1.12') || req.headers.host?.includes('localhost');
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const cookieOptions: {
      ...
      httpOnly: true,
      secure: isProduction && !req.headers.host?.includes('172.28.91.10') && !req.headers.host?.includes('10.91.1.12') && !req.headers.host?.includes('localhost'),
      sameSite: 'lax',
      maxAge: 1 * 60 * 60 * 1000,
      path: '/'
    };
