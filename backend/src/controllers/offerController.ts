...
export class OfferController {
  ...
  static async createOffer(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      // Auto-generate or validate custom offer reference number
      let finalOfferRef = req.body.offerReferenceNumber;
      if (finalOfferRef) {
        const existingOffer = await prisma.offer.findUnique({
          where: { offerReferenceNumber: finalOfferRef }
        });
        if (existingOffer) {
          return res.status(400).json({ error: 'An offer with this reference number already exists' });
        }
      } else {
        finalOfferRef = await OfferController.generateOfferReferenceNumber(
          parseInt(zoneId),
          productType,
          req.user.id
        );
      ...
      const offer = await prisma.offer.create({
        data: {
          // Basic Information
          offerReferenceNumber: finalOfferRef,
      ...
      // Create OfferSparePart entries if spare parts provided
  ...
  static async updateOffer(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      // Check if offerReferenceNumber is being updated and validate uniqueness
      if (updates.offerReferenceNumber && updates.offerReferenceNumber !== existingOffer.offerReferenceNumber) {
        const duplicateOffer = await prisma.offer.findUnique({
          where: { offerReferenceNumber: updates.offerReferenceNumber }
        });
        if (duplicateOffer) {
          return res.status(400).json({ error: 'An offer with this reference number already exists' });
        }
