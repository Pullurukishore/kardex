...
export const validatePin = async (req: Request, res: Response) => {
  try {
    ...
    const cookieOptions = {
      ...
      secure: process.env.NODE_ENV === 'production' && !req.headers.host?.includes('172.28.91.10') && !req.headers.host?.includes('10.91.1.12') && !req.headers.host?.includes('localhost'),
