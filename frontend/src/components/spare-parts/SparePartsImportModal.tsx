...
export default function SparePartsImportModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg bg-white rounded-3xl overflow-hidden p-0 border-0 shadow-2xl">
        <DialogHeader className="bg-slate-50 p-6 border-b">
          <DialogTitle className="text-xl font-bold flex items-center gap-3 text-slate-800">
            <Upload className="w-5 h-5 text-[#6F8A9D]" />
            Bulk Import Parts
          </DialogTitle>
        ...
        <div className="p-6 space-y-4">
          {!result ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="flex items-center justify-between p-3.5 bg-blue-50/50 border border-blue-100 rounded-2xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-blue-800">Need the template?</p>
                      <p className="text-[10px] text-blue-500">Download the formatted Excel file.</p>
                    </div>
                  </div>
                  <Button variant="outline" size="sm" onClick={onDownloadTemplate} className="bg-white border-blue-200 text-blue-700 hover:bg-blue-100 h-9 px-3 rounded-xl text-xs font-bold">
                    <Download className="w-3.5 h-3.5 mr-1.5" /> Template
                  </Button>
                </div>

                <div className={`relative border-2 border-dashed rounded-3xl p-6 transition-all ${file ? 'bg-green-50/30 border-green-200' : 'bg-slate-50 border-slate-200 hover:border-[#6F8A9D]/50'}`}>
                  <input
                    type="file"
                  ...
                  <div className="flex flex-col items-center justify-center text-center">
                    <div className={`p-3 rounded-2xl mb-3 ${file ? 'bg-green-100 text-green-600' : 'bg-slate-200 text-slate-400'}`}>
                      {file ? <FileCheck className="w-8 h-8" /> : <Upload className="w-8 h-8" />}
                    </div>
                    <div>
                      <p className={`text-base font-bold ${file ? 'text-green-800' : 'text-slate-700'}`}>
                        {file ? file.name : 'Click or Drag to Upload'}
                      </p>
                      <p className="text-xs text-slate-400 mt-0.5">Excel or CSV files only</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="text-center py-10 space-y-4">
              ...
              <Button onClick={() => onOpenChange(false)} className="bg-slate-800 h-11 px-8 rounded-xl mt-5">Close</Button>
            </div>
          )}
        </div>

        {!result && (
          <DialogFooter className="bg-slate-50 p-6 pt-0 gap-3 sm:gap-0">
            <Button variant="outline" onClick={() => onOpenChange(false)} className="rounded-xl h-11 flex-1">Cancel</Button>
            {previewData ? (
              <Button onClick={handleExecute} disabled={importing} className="rounded-xl h-11 flex-1 bg-green-600 hover:bg-green-700 shadow-lg text-white font-bold">
                {importing ? <RefreshCw className="h-5 w-5 animate-spin" /> : 'Execute Import'}
              </Button>
            ) : (
              <Button onClick={handlePreview} disabled={!file || previewing} className="rounded-xl h-11 flex-1 bg-[#6F8A9D] hover:bg-[#546A7A] shadow-lg text-white font-bold">
