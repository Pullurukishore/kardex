...
export default function AIOfferInsights({ defaultExpanded = false }: { defaultExpanded?: boolean }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const result = await apiService.aiChat(msg);
  ...
  const quickQuestions = [
    {
      ...
      q: "Give me a quick pipeline health summary",
    },
    { label: "🏆 Top Zones", q: "Which zone has the best win rate and why?" },
    { label: "📉 At Risk", q: "Which offers or zones are at risk right now?" },
    {
      label: "🎯 Target Gap",
      q: "How are we tracking against our yearly targets?",
    },
    {
      label: "💡 Recommendations",
      q: "What should I focus on this week to improve sales?",
    },
  ];
        ...
        <CardContent className="p-8">
          <div className="flex flex-col items-center text-center max-w-md mx-auto">
            ...
            <h3 className="text-xl font-bold text-[#546A7A] mb-2">
              AI Intelligence Not Configured
            </h3>
            <p className="text-[#5D6E73] mb-6 leading-relaxed">
              To enable AI-powered insights, please add your{" "}
        ...
        {chatMessages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center w-full max-w-5xl mx-auto px-2">
            <div className="h-20 w-20 rounded-3xl bg-gradient-to-br from-[#546A7A]/10 to-[#6F8A9D]/10 flex items-center justify-center mb-6 shadow-sm border border-white">
              <Brain className="h-10 w-10 text-[#546A7A]" />
            </div>
            <h2 className="text-2xl font-bold text-[#546A7A] mb-2">
              How can I help you analyze the offers today?
            </h2>
            <p className="text-[#92A2A5] mb-8">
              Ask about pipeline health, win rates, or get recommendations based on current data.
            </p>
      ...
      <div className="border-t border-[#AEBFC3]/20 p-4 bg-gray-50 flex-shrink-0">
        <div className="w-full flex gap-3 max-w-5xl mx-auto">
          <Input
            ...
            placeholder="Ask about pipeline health, zone win rates, target gaps..."
