...
interface DetailedContractCreateFormProps {
  role: string;
  backUrl: string;
}

export default function DetailedContractCreateForm({ role, backUrl }: DetailedContractCreateFormProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const zArr = Array.isArray(zData) ? zData : (zData?.data || []);
        setZones(zArr);
        if (zArr.length > 0) {
          setZoneName(zArr[0].name);
          setSelectedZoneId(String(zArr[0].id));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiService.createDetailedContract(payload);
      toast.success('Machine contract created successfully!');
      router.push(backUrl);
    } catch (err: any) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 border border-white/20 mb-2 backdrop-blur-md">
                <Plus className="w-3.5 h-3.5 text-[#82A094]" />
                <span className="text-[11px] font-bold text-white tracking-wider uppercase">
                  New Annual Machine Contract
                </span>
              </div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
                Register Machine Contract
              </h1>
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-4 sm:p-6 shadow-sm flex items-center justify-between gap-4">
          ...
          >
            ...
            <span>{submitting ? 'Creating Contract...' : 'Create Contract'}</span>
          </button>
        </div>
      </form>
    </div>
