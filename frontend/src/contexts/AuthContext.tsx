  ...
  const loadUser = useCallback(async (currentPath?: string): Promise<User | null> => {
    ...
    try {
      ...
      setCookie('userRole', safeUser.role, {
        path: '/',
        secure: process.env.NODE_ENV === 'production' && !window.location.hostname.includes('172.28.91.10') && !window.location.hostname.includes('10.91.1.12') && !window.location.hostname.includes('10.91.1.48') && !window.location.hostname.includes('10.91.1.49') && !window.location.hostname.includes('10.91.1.22') && window.location.hostname !== 'localhost',
        sameSite: 'lax',
        maxAge: 60 * 60 * 24 * 30,
      });
